...
int calcCost(struct tile* map, int sizeX, int sizeY, int myBelt, struct toCheck thisPoint) {
  ...
  if (thisPoint.goingDown || thisPoint.underground || thisPoint.goingUp) {
    cost++;
  }
...
int findCost(struct tile* map, int myBelt, int sizeX, int sizeY, struct toCheck startingPoint, int* whichX, int* whichY, bool* isUnder) {
  ...
  for (int i =0; i < numDirections; i++) {
    ...
    if (direction != startingPoint.direction) {
      ...
      // TODO: only do the move if the cost looks right!
      if (validPoint(newX, newY, sizeX, sizeY)) {
        struct tile thisTile = tileAt(map, sizeX, sizeY, newX, newY);
        // add cost if there is another belt endpoint nearby
        int thisCost;
        bool thisIsUnder = false;
        if (startingPoint.goingUp) {
          thisIsUnder = true;
          thisCost = thisTile.underCost;
          debug_print_routing("stayingUnder Cost of %d, %d in dir: %d is: %d\n", startingPoint.x, startingPoint.y, direction, thisCost);

        } else if (startingPoint.underground) {
          //tunnel entrance
          thisCost = thisTile.underCost;
          if (thisTile.goingDownCost <= thisTile.underCost) {
            thisCost = thisTile.goingDownCost;
            thisIsUnder = false;
            debug_print_routing("GoingDown Cost of %d, %d in dir: %d is: %d\n", startingPoint.x, startingPoint.y, direction, thisCost);
            //thisCost = thisTile.cost;
          } else {
            thisIsUnder = true;
            debug_print_routing("Underground Cost of %d, %d in dir: %d is: %d\n", startingPoint.x, startingPoint.y, direction, thisCost);
          }
        } else { //goingDown or aboveground
          thisIsUnder = false;
          thisCost = thisTile.cost;
          if (thisTile.goingUpCost <= thisTile.cost) {
            // TODO: only go down if there is an obstacle in the way!!!
            if (obstacleAhead(map, sizeX, sizeY, newX, newY, direction)) {
              thisIsUnder = true;
            }
            debug_print_routing("Aboveground GoingUp cost of %d, %d in dir: %d is: %d\n", startingPoint.x, startingPoint.y, direction, thisCost);

          } //else {
            debug_print_routing("Aboveground Cost of %d, %d in dir: %d is: %d\n", startingPoint.x, startingPoint.y, direction, thisCost);
          //}
        }
...
int routeNet(struct tile* map, int myBelt, int sizeX, int sizeY, struct net net) {
  ...
  while (!toCheckList.empty()) {
    ...
    if (checkPoint(map, myBelt, sizeX, sizeY, currentPoint)){
      ...
      // this is our solution! (if it isn't underground)
      if (currentPoint.x == net.endX && currentPoint.y == net.endY && (!currentPoint.underground)) {
      ...
      if (currentPoint.goingDown) {
        ...
        if (currentPoint.cost <= thisTile->goingDownCost) {
          thisTile->goingDownCost = currentPoint.cost;
        ...
        if (currentPoint.cost <= thisTile->goingUpCost) {
          thisTile->goingUpCost = currentPoint.cost;
          if (currentPoint.cost <= thisTile->cost) {
            thisTile->cost = currentPoint.cost;// TODO: do i need this?
          }
          //thisTile->whichBelt = myBelt; // TODO: is this nescesary anymore?
          addAllOptions(map, &toCheckList, sizeX, sizeY, currentPoint);
        }
  ...
  if (finalPoint.cost != -1) {
    ...
    while (currentPoint.x != net.startX || currentPoint.y != net.startY) {
      //update last point
      ...
      } else if (isUnder && (currentPoint.goingUp || currentPoint.underground)) {
        // UNDER GROUND
    ...
    map[net.endX + (net.endY * sizeX)].whichBelt = myBelt;
    map[net.endX + (net.endY * sizeX)].hasBelt = true;
    map[net.endX + (net.endY * sizeX)].beltDirection = finalPoint.direction;
